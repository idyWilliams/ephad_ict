...
export const Footer = () => {
  return (
    <footer className="bg-[var(--lm-panel)] dark:bg-[#02040a] border-t border-[var(--lm-ink)]/10 dark:border-white/10 pt-24 pb-8 overflow-hidden relative z-10 transition-colors duration-500">
      <div className="container mx-auto px-6 md:px-[10%] relative z-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 md:gap-8 pb-20 border-b border-[var(--lm-ink)]/10 dark:border-white/10 transition-colors duration-500">
          ...
          <div className="md:col-span-5 flex flex-col justify-between">
            <div>
              <Link href="/" className="inline-flex items-center gap-4 group mb-6">
                <div className="w-12 h-12 border border-[var(--lm-ink)]/20 dark:border-white/20 flex items-center justify-center text-[var(--lm-ink)] dark:text-white font-bold text-xl group-hover:bg-[var(--lm-ink)] group-hover:text-[var(--lm-base)] dark:group-hover:bg-white dark:group-hover:text-[var(--lm-ink)] transition-all">
                  E.
                </div>
              ...
              <p className="text-[var(--lm-body)]/50 dark:text-white/40 font-light max-w-sm text-sm leading-relaxed mb-8 transition-colors duration-500">
          ...
          <div className="md:col-span-3 md:col-start-8">
            <h4 className="text-[10px] font-semibold uppercase tracking-[0.3em] text-[var(--lm-body)]/30 dark:text-white/30 mb-8 transition-colors duration-500">Navigation</h4>
            <ul className="flex flex-col gap-4">
              {['Syllabus', 'Intel', 'Intercepts', 'Registration'].map((item) => (
                <li key={item}>
                  <Link href={item === 'Registration' ? '/#register' : `/#${item.toLowerCase()}`} className="text-[var(--lm-body)]/60 dark:text-white/60 hover:text-[var(--lm-ink)] dark:hover:text-white uppercase tracking-widest text-sm font-light transition-colors">
                    {item}
                  </Link>
                </li>
              ))}
          ...
          <div className="md:col-span-2">
            <h4 className="text-[10px] font-semibold uppercase tracking-[0.3em] text-[var(--lm-body)]/30 dark:text-white/30 mb-8 transition-colors duration-500">Network</h4>
            <ul className="flex flex-col gap-4">
        ...
        <div className="py-12 md:py-20 flex justify-center border-b border-[var(--lm-ink)]/10 dark:border-white/10 mb-8 transition-colors duration-500">
          <span className="text-[15vw] md:text-[18vw] font-black uppercase leading-none tracking-tighter text-[var(--lm-ink)]/5 dark:text-white/5 select-none transition-colors duration-500">
            EPHAD
          </span>
        </div>

        {/* Bottom Bar */}
        <div className="flex flex-col items-center justify-between gap-4 text-[10px] uppercase tracking-[0.2em] font-medium text-[var(--lm-body)]/30 dark:text-white/30 md:flex-row transition-colors duration-500">
          ...
          <p>
            ...
            <a href="https://isentrytechnologies.com" target="_blank" rel="noopener noreferrer" className="text-[var(--lm-ink)] dark:text-white hover:text-[var(--color-brand-sky)] transition-colors border-b border-[var(--lm-ink)]/20 dark:border-white/20 hover:border-transparent">
